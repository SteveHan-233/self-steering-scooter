...
<mujoco>
    ...
    <worldbody>
        <body pos="0 0 0" euler="0 0 0" mocap="true" name="camera">
            <camera name="top_view" fovy="60" pos="0 0 90" />
        </body>
...
class Sim(gym.Env):
    ...
    def reset(self):
        ...
        #angle = -1.14
        scooter_x = np.random.uniform(-20, 20)
        scooter_y = np.random.uniform(-5, 5)
    ...
    def render_episode(self, model, render_name, max_time=10, num_episodes=1):
        for i in range(num_episodes):
            ...
            while self.physics.data.time < max_time and not done:
                ...
                if num_frames < self.physics.data.time * framerate / (15 * (i + 1)**.7):
                    self.physics.named.data.mocap_pos['camera'] = np.array([0, 0, min(10 + len(self.frames)/100 * 40, 150)])
