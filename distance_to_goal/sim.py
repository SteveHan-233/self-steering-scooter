...
<mujoco>
    ...
    <worldbody>
        ...
        <body>
            <geom type="cylinder" pos="-50 0 1" size=".3 1"/>
        </body>
    </worldbody>
    <actuator>
...
class Sim(gym.Env):
    def __init__(self, hertz, max_timestep=2000):
        ...
        high = np.array([.1, np.finfo(np.float32).max, .5, .5, 1, np.finfo(np.float32).max])
    ...
    def reset(self):
        ...
        #angle = np.random.uniform(1, math.pi/2)
        angle = 1.5
    ...
    def get_goal_distance(self):
        scooter_frontwheel_pos = self.physics.named.data.xpos['front_wheel'][:2]
        return np.linalg.norm(self.goal_pos - scooter_frontwheel_pos)
    ...
    def get_state(self):
        return np.array([self.get_tilt_angle(), self.get_tilt_velocity(), self.get_steer_angle(), self.get_steer_goal(), self.get_goal_angle(), self.get_goal_distance()], dtype=np.float32)
